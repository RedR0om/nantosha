...
const form = useForm({
    ...
    media_type: props.slide.media_type || 'image',
...
const imagePreview = ref<string | null>(props.slide.image);
const videoPreview = ref<string | null>(props.slide.video);
...
const removeImage = () => {
    form.image = null;
    imagePreview.value = props.slide.image;
};

const removeVideo = () => {
    form.video = null;
    if (videoPreview.value && videoPreview.value !== props.slide.video) {
        URL.revokeObjectURL(videoPreview.value);
    }
    videoPreview.value = props.slide.video;
};

const submit = () => {
    form.put(`/admin/carousel/${props.slide.id}`, {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <AppLayout>
        <Head title="Edit Carousel Slide - Admin" />
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-8">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 tracking-tight mb-2">Edit Carousel Slide</h1>
                            <p class="text-gray-600">Update carousel slide information</p>
                        </div>
                        <Link
                            href="/admin/carousel"
                    ...
                    <div class="space-y-6">
                        ...
                        <div v-if="form.media_type === 'video'">
                            ...
                            <label class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 hover:border-gray-400 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <Upload class="w-8 h-8 text-gray-400 mb-2" />
                                    <p class="text-sm text-gray-600">Click to upload new video</p>
                                    <p class="text-xs text-gray-400 mt-1">MP4, MOV, AVI, WebM up to 50MB</p>
                                </div>
                                <input
                                    type="file"
                                    accept="video/*"
