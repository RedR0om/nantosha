...
const form = useForm({
    type: props.section?.type || 'hero',
    title: props.section?.title || '',
    subtitle: props.section?.subtitle || '',
    content: props.section?.content || null,
    background_color: props.section?.background_color || 'white',
...
// Initialize form data from existing section content
onMounted(() => {
    if (props.section?.content) {
        const content = props.section.content;
        
        if (form.type === 'hero') {
            heroBadges.value = content.badges || [];
            heroButtons.value = content.buttons || [];
        } else if (form.type === 'features') {
            featuresItems.value = content.items || [];
        } else if (form.type === 'product_info') {
            ingredients.value = content.ingredients || [];
            details.value = content.details || [];
            indications.value = content.indications || '';
        } else if (form.type === 'pricing') {
            pricingTiers.value = content.tiers || [];
            pricingNote.value = content.note || '';
        } else if (form.type === 'how_it_works') {
            steps.value = content.steps || [];
            linkText.value = content.linkText || '';
            linkHref.value = content.linkHref || '';
            ...
            customContent.value = {
                ...
                steps: content.steps || [],
                step_link_text: content.step_link_text || '',
                step_link_href: content.step_link_href || '',
                custom_css: content.custom_css || '',
                spacing: content.spacing || 'normal',
                max_width: content.max_width || 'full',
                text_alignment: content.text_alignment || 'left'
            };
        }
    }
});
...
watch(() => form.type, () => {
    ...
    // Reload data if section content exists for new type
    if (props.section?.content) {
        const content = props.section.content;
        if (form.type === 'hero') {
            heroBadges.value = content.badges || [];
...
const submit = () => {
    ...
    form.put(`/admin/homepage-sections/${props.section.id}`, {
        preserveScroll: true,
        onError: (errors) => {
...
<template>
    <AppLayout>
        <Head title="Edit Homepage Section - Admin" />
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-8">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 tracking-tight mb-2">Edit Homepage Section</h1>
                            <p class="text-gray-600">Update homepage content section</p>
                        </div>
                        <Link
                ...
                <form @submit.prevent="submit" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-8">
                    <div class="space-y-6">
                        <!-- Section Type -->
                        <div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows="3"
                                class="w-full border border-gray-300 dark:border-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white bg-white dark:bg-gray-800 focus:outline-none focus:border-gray-900 dark:focus:border-gray-600 rounded-lg"
                                placeholder="e.g., Genuine Ivermectin Capsules (15mg) - Verified by Japanese Research Institutions"
                            ></textarea>
                            <p class="mt-1 text-xs text-gray-500">Content will be automatically translated to Japanese when viewed</p>
