...
const deleteContent = (id: number) => {
    if (confirm('Are you sure you want to delete this content?')) {
        router.delete(`/admin/corporate-profile/${id}`, {
            preserveScroll: true,
        });
    }
};
...
const toggleActive = (content: any) => {
    router.put(`/admin/corporate-profile/${content.id}`, {
        ...content,
...
<template>
    <AppLayout>
        <Head title="Corporate Profile Content - Admin" />
        
        <div class="py-8 bg-gray-50 min-h-screen">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Corporate Profile Content</h1>
                        <p class="mt-1 text-sm text-gray-600">Manage content for the Corporate Profile page</p>
                    </div>
                    <Link
                        href="/admin/corporate-profile/create"
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-3 mb-2">
                                            ...
                                            <span v-if="content.field_type" class="px-2 py-1 text-xs font-semibold bg-purple-100 text-purple-800 rounded">
                                                {{ content.field_type }}
                                            </span>
                                        </div>
                                        <h3 class="text-base font-semibold text-gray-900 mb-1">
                                            {{ content.title || content.label || '(No title)' }}
                                        </h3>
                                        <p v-if="content.subtitle" class="text-sm text-gray-600 mb-2">
                                            {{ content.subtitle }}
                                        </p>
                                        <p v-if="content.label && content.value" class="text-sm text-gray-600 mb-1">
                                            <span class="font-medium">{{ content.label }}:</span> {{ content.value }}
                                        </p>
                                        <p v-if="content.text" class="text-sm text-gray-600 mb-2 line-clamp-2">
                                            {{ content.text }}
                                        </p>
                                        <p v-if="content.value && !content.label" class="text-sm text-gray-600 mb-2">
                                            {{ content.value }}
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-2 ml-4">
                                        ...
                                        <Link
                                            :href="`/admin/corporate-profile/${content.id}/edit`"
