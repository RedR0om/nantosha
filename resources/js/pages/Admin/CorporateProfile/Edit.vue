...
const form = useForm({
    ...
    field_type: props.content.field_type || '',
...
// Load existing office location fields
onMounted(() => {
    if (props.content.section_type === 'office_location' && props.relatedContents && props.relatedContents.length > 0) {
        officeContactFields.value = props.relatedContents.map(c => ({
            id: c.id,
            label: c.label || '',
            value: c.value || '',
    ...
    } else if (props.content.section_type === 'office_location') {
        // If no related contents, initialize with current content
...
const submit = () => {
    // If office_location with multiple fields, handle bulk update
    if (form.section_type === 'office_location' && officeContactFields.value.length > 0) {
        ...
        // Get IDs of existing entries to delete
        const existingIds = officeContactFields.value
            .filter(f => f.id)
            .map(f => f.id!);
        ...
        const bulkForm = useForm({
            ...
            delete_ids: existingIds.filter(id => !fieldsToSubmit.some(f => f.id === id)),
        });

        bulkForm.put(`/admin/corporate-profile/${props.content.id}`, {
            preserveScroll: true,
        });
    } else {
        // Regular single entry submission
        form.put(`/admin/corporate-profile/${props.content.id}`, {
            preserveScroll: true,
        });
    }
};
...
<template>
    <AppLayout>
        <Head title="Edit Corporate Profile Content - Admin" />
        ...
        <div class="py-8 bg-gray-50 min-h-screen">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Edit content for the Corporate Profile page</p>
                </div>
