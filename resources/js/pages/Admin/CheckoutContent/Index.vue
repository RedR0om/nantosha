...
const deleteContent = (id: number) => {
    if (confirm('Are you sure you want to delete this content?')) {
        router.delete(`/admin/checkout-content/${id}`, {
            preserveScroll: true,
        });
    }
};
...
const toggleActive = (content: any) => {
    router.put(`/admin/checkout-content/${content.id}`, {
        ...content,
...
const getSectionTypeLabel = (type: string) => {
    const labels: { [key: string]: string } = {
        risks_prohibitions: 'Risks & Prohibitions',
        shipping_customs: 'Shipping & Customs Information',
    };
...
<template>
    <AppLayout>
        <Head title="Checkout Content - Admin" />
        
        <div class="py-8 bg-gray-50 min-h-screen">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Checkout Content</h1>
                        <p class="mt-1 text-sm text-gray-600">Manage content for the Checkout page</p>
                    </div>
                    <Link
                        href="/admin/checkout-content/create"
                            ...
                            >
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <p v-if="content.content" class="text-sm text-gray-600 mb-2 line-clamp-3">
                                            {{ content.content }}
                                        </p>
                                        <p v-if="content.description" class="text-sm text-gray-500 mb-2 line-clamp-2">
                                            {{ content.description }}
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-2 ml-4">
                                        ...
                                        <Link
                                            :href="`/admin/checkout-content/${content.id}/edit`"
