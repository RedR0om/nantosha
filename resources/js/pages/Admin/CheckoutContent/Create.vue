...
// Reset items when section type changes
watch(() => form.section_type, (newType) => {
    if (newType === 'risks_prohibitions') {
        riskItems.value = [];
        prohibitionItems.value = [];
        acknowledgmentItems.value = [];
    } else {
        // Reset form fields for shipping_customs
        form.key = '';
        form.title = '';
        form.content = '';
        form.description = '';
...
const submit = () => {
    ...
    if (form.section_type === 'risks_prohibitions') {
        ...
        if (allItems.length === 0 || (riskItems.value.length === 0 && prohibitionItems.value.length === 0 && acknowledgmentItems.value.length === 0)) {
            alert('Please add at least one risk, prohibition, or acknowledgment item');
            return;
        }
        ...
        bulkForm.post('/admin/checkout-content', {
            preserveScroll: true,
        });
    } else {
        // Regular single entry submission for shipping_customs
        form.post('/admin/checkout-content', {
            preserveScroll: true,
        });
    }
};
...
<template>
    <AppLayout>
        <Head title="Create Checkout Content - Admin" />
        ...
        <div class="py-8 bg-gray-50 min-h-screen">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Add new content to the Checkout page</p>
                </div>
